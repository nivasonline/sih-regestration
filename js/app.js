...
function nextStep(fromStep) {
  ...
  // Step 4 is now automated via payWithRazorpay().
  // The user shouldn't reach nextStep(4) directly unless payment is verified.
...
async function payWithRazorpay() {
  ...
  try {
    let razorpayKey = "rzp_test_TXfG9hOcD2aYvP"; // Fallback for static frontend deployment
    let orderId = "order_static_12345"; // Dummy order ID for static preview

    try {
      // 1. Try to create order and fetch config on backend (will fail on static GitHub Pages)
      ...
      console.warn("Backend not found, using static preview mode for Razorpay KYC");
    }
    ...
    const options = {
      ...
      handler: function (response) {
        ...
        // Show and click continue
        const continueBtn = document.getElementById('btn-payment-continue');
...
function goToReview() {
  ...
  // Payment
  const reviewTx = document.getElementById('review-transaction');
  if (reviewTx) reviewTx.textContent = formData.transactionId;
